...
const step = ref<'choice' | 'form' | 'list' | 'manage' | 'link'>(
...
async function handleCreateTeam() {
  ...
  try {
    ...
    // Не закрываем модалку, она сама переключится на 'manage'
  } catch (error: any) {
...
async function handleDisbandTeam() {
  ...
  modalConfig.message = `Вы уверены, что хотите расформировать команду "${props.currentParticipation.team_name}"?`;
  showActionModal.value = true;
}
...
<template>
  <div class="p-6">
    ...
    <div v-if="currentParticipation">
      ...
      <div v-if="isCaptain" class="mb-6">
        ...
        <div class="relative">
          ...
          <button @click="copyLink" :title="copyButtonText" class="absolute top-1/2 right-1 -translate-y-1/2 text-gray-500 hover:text-primary p-1"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>
        </div>
      </div>
      <div>
        <h3 class="font-semibold mb-2">Участники команды ({{ currentParticipation.members.length }})</h3>
        <ul class="space-y-2">
          <li v-for="member in currentParticipation.members" :key="member.user.id" class="flex items-center justify-between p-2">
            <div class="flex items-center gap-3"><img :src="member.user.avatarUrl || '/img/icons/default-avatar.svg'" class="w-8 h-8 rounded-full object-cover"><span class="font-medium">{{ member.user.fullName }}</span></div>
            <button v-if="isCaptain && authStore.user?.id !== member.user.id" @click="handleKickMember(member.user.id)" class="text-xs text-red-500 hover:underline">Удалить</button>
          </li>
        </ul>
      </div>
      <div class="mt-6 pt-6 border-t">
        <button v-if="isCaptain" @click="handleDisbandTeam" class="w-full text-center p-3 text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">Расформировать команду</button>
    ...
    <!-- VIEW 2: Пользователь НЕ в команде -->
    <div v-else>
      ...
      <div v-if="step === 'choice'">
        ...
        <div class="flex flex-col gap-4">
          ...
          <button @click="step = 'form'" class="w-full text-center p-3 bg-gray-100 rounded-lg hover:bg-gray-200">Создать команду</button>
        </div>
      </div>
        ...
        <div class="border rounded-lg">
          ...
          <ul class="space-y-1 max-h-64 overflow-y-auto p-2">
            <li v-for="team in filteredTeams" :key="team.id" class="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg">
              ...
              <button @click="handleJoinTeam(team.id)" :disabled="isLoading" class="px-4 py-1 text-sm bg-primary text-white rounded-full hover:opacity-90 disabled:bg-gray-300">Вступить</button>
            </li>
