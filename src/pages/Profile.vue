...
<template>
  <div class="p-4 max-w-4xl mx-auto">
    <div v-if="authStore.isAuthenticated && authStore.user">
          ...
          <div>
            ...
            <div class="flex items-center gap-2 mt-1 group cursor-pointer" @click="copyUserId">
              ...
              <div class="relative">
                ...
                <span class="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap text-xs text-white bg-gray-700 px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                  {{ copyIdText }}
                </span>
              </div>
            </div>
      ...
      <div v-if="registeredEvents.length > 0" class="space-y-3"><EventCard v-for="event in registeredEvents" :key="event.id" :event="event" /></div>
    ...
    <div v-else>
      <div class="bg-white p-8 rounded-lg shadow-md mt-10">
        <form v-if="loginStep === 'credentials'" @submit.prevent="handleLogin" class="space-y-4">
          ...
          <div><label for="login-email" class="block text-sm font-medium text-gray-700">Email или ID</label><input v-model="loginForm.email" type="text" id="login-email" required autocomplete="username" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
          <div><label for="login-password" class="block text-sm font-medium text-gray-700">Пароль</label><input v-model="loginForm.password" type="password" id="login-password" required autocomplete="current-password" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
        ...
        <form v-if="loginStep === '2fa'" @submit.prevent="handle2FA" class="space-y-4">
          <h2 class="text-xl font-bold text-center mb-2">Подтверждение входа</h2><p class="text-center text-sm text-gray-600 mb-4">Мы отправили код на вашу почту.</p>
          <div><label for="2fa-code" class="block text-sm font-medium text-gray-700">Код подтверждения</label><input v-model="twoFaForm.code" type="text" id="2fa-code" required class="mt-1 w-full text-center tracking-[0.5em] px-3 py-2 border border-gray-300 rounded-md"></div>
          <p v-if="loginError" class="text-sm text-red-600 pt-2">{{ loginError }}</p>
          <button type="submit" :disabled="loginIsLoading" class="w-full py-2 px-4 text-white bg-primary rounded-md disabled:bg-gray-400">Подтвердить</button>
        </form>
    ...
    <Modal :show="showSettings" @close="showSettings = false">
      <div class="p-6">
        ...
        <div v-if="activeSettingsTab === 'personal'" class="space-y-4">
          <div class="flex items-center gap-4"><img :src="avatarPreview || authStore.userAvatar" class="w-20 h-20 rounded-full object-cover"><button @click="triggerAvatarUpload" type="button" class="px-4 py-2 text-sm bg-gray-100 rounded-lg hover:bg-gray-200 border">Сменить фото</button><input type="file" ref="avatarInput" @change="onFileSelected" class="hidden" accept="image/*"></div>
          <div><label class="block text-sm font-medium">ФИО</label><input v-model="profileForm.fullName" class="mt-1 w-full px-3 py-2 border rounded-md"></div>
          <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium">Рост (см)</label><input v-model="profileForm.height" type="number" class="mt-1 w-full px-3 py-2 border rounded-md"></div><div><label class="block text-sm font-medium">Вес (кг)</label><input v-model="profileForm.weight" type="number" class="mt-1 w-full px-3 py-2 border rounded-md"></div></div>
          <div><label class="block text-sm font-medium">Дата рождения</label><input v-model="profileForm.birthday" type="date" class="mt-1 w-full px-3 py-2 border rounded-md"></div>
          <div><label class="block text-sm font-medium">Пол</label><select v-model="profileForm.gender" class="mt-1 w-full px-3 py-2 border rounded-md bg-white"><option disabled value="">Выберите пол</option><option value="male">Мужской</option><option value="female">Женский</option></select></div>
          <button @click="handleProfileSave" class="w-full mt-4 px-4 py-2 text-white bg-primary rounded-md">Сохранить</button>
        </div>
