...
const nextImage = () => { 
  if (eventImages.value.length > 0) currentImageIndex.value = (currentImageIndex.value + 1) % eventImages.value.length 
}
const prevImage = () => { 
  if (eventImages.value.length > 0) currentImageIndex.value = (currentImageIndex.value - 1 + eventImages.value.length) % eventImages.value.length 
}
...
<template>
  <div v-if="event" class="bg-bgMain min-h-full">
    <div class="relative w-full h-64 bg-gray-300">
      <img 
      :src="eventImages[0] || defaultImage" 
      alt="Event visual" 
      ...
      <button v-if="eventImages.length > 0" @click="openViewer(0)" class="absolute bottom-4 right-4 bg-black/50 text-white p-2 rounded-full hover:bg-black/75 transition-colors">
    ...
    <div class="max-w-md mx-auto">
      <div class="flex justify-between items-center mb-4 text-center">
        ...
        <div>
          ...
          <p class="font-bold text-gray-800">{{ event.date }}</p>
        </div>
        
        <!-- Время начала (если доступно) -->
        <div v-if="event.start_time">
          <p class="text-sm text-gray-500">Начало</p>
          <p class="font-bold text-gray-800">{{ event.start_time }}</p>
        </div>
        
        <!-- Участники -->
        <div v-if="!event.is_team">
          <p class="text-sm text-gray-500">Участников</p>
          <p class="font-bold text-gray-800">{{ event.max_members || 'Неограничено' }}</p>
        </div>
        
        <!-- Команды (только для командных событий) -->
        <div v-if="event.is_team">
          <p class="text-sm text-gray-500">Команд</p>
          <p class="font-bold text-gray-800">{{ event.max_teams || 'Неограничено' }}</p>
        </div>
      </div>
      
      <button 
        @click="handleParticipate" 
        class="w-full bg-primary text-white font-bold py-3 rounded-lg text-lg hover:opacity-90 transition-opacity"
      >
        {{ authStore.isAuthenticated ? 'Участвовать' : 'Войти, чтобы участвовать' }}
      </button>
    </div>
  </footer>
