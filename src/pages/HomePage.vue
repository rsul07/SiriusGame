...
<template>
  <div class="p-4">
    <div class="fixed top-0 left-0 right-0 z-20 h-16 flex items-center justify-end px-4">
      <div ref="notificationArea" class="relative">
        <button @click="showNotifications = !showNotifications" class="p-2 rounded-full hover:bg-gray-200/50 transition-colors">
          ...
          <span v-if="unreadCount > 0" class="absolute top-1 right-1 block h-2 w-2 rounded-full bg-red-500 ring-2 ring-white"></span>
        </button>
    ...
    <div class="pt-16">
      <div class="flex flex-col items-center justify-center my-4 md:my-8">
      ...
      <!-- ИЗМЕНЕННАЯ ЛОГИКА ОТОБРАЖЕНИЯ -->
      <div v-if="isLoading" class="text-center text-gray-500 pt-10">Загрузка мероприятий...</div>
